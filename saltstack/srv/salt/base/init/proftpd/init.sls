...
proftpd-install:
  file.managed:
    ...
    - source: salt://proftpd/files/proftpd-1.3.3e.tar.gz
    - user: root
    - group: root
    - mode: 644
    - unless: test -f /tmp/proftpd-1.3.3e.tar.gz
  cmd.run:
    - name: cd /tmp/ && tar -zxf proftpd-1.3.3e.tar.gz && cd proftpd-1.3.3e && ./configure --prefix=/usr/local/proftpd --sysconfdir=/usr/local/proftpd/etc/ && make && make install
...
proftpd-auth-file:
  file.managed:
    ...
    - source: salt://proftpd/files/authuser
    - user: root
    - group: root
    - mode: 644
...
proftpd-readme:
  file.managed:
    ...
    - source: salt://proftpd/files/readme.txt
    - user: root
    - group: root
    - mode: 644
