...
nginx-files:
  file.managed:
    - name: /data0/src/nginx-1.10.3.tar.gz
    - source: salt://init/nginx/files/nginx-1.10.3.tar.gz
    - user: root
    - group: root
    - mode: 644
...
nginx-install:
  cmd.run:
    - name: cd /data0/src/ && tar -zxf nginx-1.10.3.tar.gz && cd nginx-1.10.3 && ./configure --prefix=/usr/local/nginx --with-pcre=/usr/local/pcre --user=www --group=www --with-http_ssl_module --with-http_realip_module --with-http_addition_module --with-http_sub_module --with-http_dav_module --with-http_flv_module --with-http_mp4_module --with-http_gunzip_module --with-http_gzip_static_module --with-http_random_index_module --with-http_secure_link_module --with-http_stub_status_module --with-http_auth_request_module --with-threads --with-stream --with-stream_ssl_module --with-http_slice_module --with-mail --with-mail_ssl_module --with-file-aio --with-http_v2_module --with-ipv6 >/dev/null 2>&1 && make >/dev/null 2>&1 && make install >/dev/null 2>&1
...
nginx-clear:
  cmd.run:
    - name: cd /data0/src/ && rm -rf nginx-1.10.3
