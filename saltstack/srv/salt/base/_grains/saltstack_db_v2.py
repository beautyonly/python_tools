...
db = MySQLdb.connect("127.0.0.1", "root", "strong", "saltstack")  
cur = db.cursor()  
...
try:  
    for i in grains.keys():  
        ...
        #kaixin_openfalcon_endpoint = grains[i]["kaixin_openfalcon_endpoint"]  
        #serverDomain = grains[i]["serverDomain"]  
        #serverId = grains[i]["serverId"]  
        #serverName = grains[i]["serverName"]  
        #yttx_client_version = grains[i]["yttx_client_version"]  
        #yttx_db_version = grains[i]["yttx_db_version"]  
        #yttx_resource_version = grains[i]["yttx_resource_version"]  
        #yttx_server_version = grains[i]["yttx_server_version"]  
        #yttx_time_version = grains[i]["yttx_time_version"]  
